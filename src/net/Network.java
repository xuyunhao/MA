...
public class Network {
	...
	public byte[] ConnectToServerFake(String url, List<NameValuePair> content, boolean UseDefaultKey) throws InvalidKeyException, NoSuchAlgorithmException, NoSuchPaddingException, IllegalBlockSizeException, BadPaddingException, ClientProtocolException, IOException {
		List<NameValuePair> post = RequestProcess(content,UseDefaultKey);
		...
		UsernamePasswordCredentials upc = new UsernamePasswordCredentials(Auth,Key);
		cp.setCredentials(as, upc);
		for (NameValuePair p: content) {
			if (p.getName() == "serial_id") {
				System.out.println(Crypto.Encrypt2Base64WithKey(p.getValue()));
			}
			if (p.getName() == "serial_id=") {
				System.out.println(Crypto.Encrypt2Base64WithKey(p.getValue()));
			}
