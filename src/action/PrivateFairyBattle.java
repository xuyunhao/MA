...
public class PrivateFairyBattle {
	...
	public static boolean run() throws Exception {
		...
		try {
			if (Process.info.fairy.SerialId.compareTo(Explore.CurrentSerialId) == 0)
				response = Process.network.ConnectToServerFake(URL_PRIVATE_BATTLE, post, false);
			else
		...
		try {
			...
			if ((boolean)xpath.evaluate("count(//ex_fairy/rare_fairy)>0", doc, XPathConstants.BOOLEAN)) {
				...
				Explore.CurrentSerialId = Process.info.fairy.SerialId;
